...
import bcrypt # Keep this import, as it's needed for hashing

# REMOVE THIS LINE: from login import login

class Register:
    # Add an optional 'on_login_redirect' parameter to __init__
    def __init__(self, root, on_login_redirect=None):
        self.root=root
        self.root.title("Register")
        self.root.geometry("1366x768+0+0")

        self.on_login_redirect = on_login_redirect # Store the callback
        ...
        self.bg=ImageTk.PhotoImage(file=r"C:\Users\chintu\OneDrive\Desktop\student\Python_Test_Projects\Images_GUI\bgReg.jpg")
        ...
        register_btn=Button(frame,command=self.reg,text="Register",font=("times new roman",15,"bold"),bd=0,relief=RIDGE,fg="#fff",bg="#002B53",activeforeground="white",activebackground="#007ACC")
        register_btn.place(x=103,y=510,width=270,height=35)

        # Creating Button Login
        # Bind this button to the callback if it exists
        login_btn=Button(frame,text="Login",font=("times new roman",15,"bold"),bd=0,relief=RIDGE,fg="#fff",bg="#002B53",activeforeground="white",activebackground="#007ACC", command=self.go_to_login)
        login_btn.place(x=533,y=510,width=270,height=35)
        ...
        if (self.var_fname.get()=="" or self.var_lname.get()=="" or self.var_cnum.get()=="" or self.var_email.get()=="" or self.var_ssq.get()=="Select" or self.var_sa.get()=="" or self.var_pwd.get()=="" or self.var_cpwd.get()==""):
            ...
            try:
                ...
                if row!=None:
                    ...
                    plaintext_password = self.var_pwd.get()
                    hashed_password = bcrypt.hashpw(plaintext_password.encode('utf-8'), bcrypt.gensalt())

                    # IMPORTANT: Double-check your column names here (e.g., 'contact' vs 'cnum' or 'contact_no')
                    # Based on your previous error, if your database column for contact is named 'contact_no',
                    # then you should use 'contact_no' here. I'm leaving it as 'contact' for now
                    # assuming you might rename your column. If not, change 'contact' to the actual column name.
                    mycursor.execute("INSERT INTO regteach (fname, lname, contact_num, email, security_question, security_answer, pwd) VALUES (%s,%s,%s,%s,%s,%s,%s)",(
                        self.var_fname.get(),
                        self.var_lname.get(),
                        self.var_cnum.get(),
                        self.var_email.get(),
    ...
    # New method to handle going back to login
    def go_to_login(self):
        self.root.destroy() # Close the current register window
        if self.on_login_redirect:
            self.on_login_redirect() # Call the function passed from the parent/login window

# This part is only for testing register.py directly.
# In a real app, login.py would start the process.
if __name__ == "__main__":
    root=Tk()
    # If running register.py directly for testing, on_login_redirect will be None
    # If you want to simulate going back, you can pass a simple lambda: print("Go to Login")
    app=Register(root) # or Register(root, on_login_redirect=lambda: print("Login button clicked!"))
    root.mainloop()